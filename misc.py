...
def plot_contours(arr, aux, pred_aux=None, level=0.5, ax=None, **kwargs):
    if pred_aux is not None:
        if ax is None:
            _, ax = plt.subplots(1, 2, **kwargs)
        ax[0].set_xlim(left=0, right=arr.shape[0])
        ax[0].set_ylim(bottom=0, top=arr.shape[0])
        ax[0].imshow(arr, cmap=plt.cm.gray)
        xs, ys = find_edges(aux, level)
        ax[0].plot(xs, ys, 'r-')
        ax[0].set_title('ground true')
        ax[1].set_xlim(left=0, right=arr.shape[0])
        ax[1].set_ylim(bottom=0, top=arr.shape[0])
        ax[1].imshow(arr, cmap=plt.cm.gray)
        xs, ys = find_edges(pred_aux, level)
        if xs is not None:
            ax[1].plot(xs, ys, 'b-')
        ...
        ax.set_xlim(left=0, right=arr.shape[0])
        ax.set_ylim(bottom=0, top=arr.shape[0])
...
def plot_voxel_enhance(arr, arr_mask=None, figsize=10, alpha=0.1, title=None):  # zyx
    plt.figure(figsize=(figsize, figsize))
    rows = cols = int(np.sqrt(arr.shape[0])) + 1
    img_height = arr.shape[1]
    img_width = arr.shape[2]
    assert img_width == img_height
    res_img = np.zeros((rows * img_height, cols * img_width), dtype=np.uint8)
    if arr_mask is not None:
        res_mask_img = np.zeros((rows * img_height, cols * img_width), dtype=np.uint8)
...
def plot_voxel_enhance_save(img_path, arr, arr_mask=None, figsize=10, alpha=0.15, title=None):  # zyx
    plt.figure(figsize=(figsize, figsize))
...
def split_validation(df, subset, by, subset_name="subset"):
    df = df.copy()
    for sset in df[by].unique():
        length = (df[by] == sset).sum()
        df.loc[df[by] == sset, subset_name] = get_uniform_assign(length, subset)
    df[subset_name] = df[subset_name].astype(int)
    return df
